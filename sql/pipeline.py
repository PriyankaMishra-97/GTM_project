...
class SqlPath:
    ...
    def __init__(
        ...
        self.generator = generator or SqlGenerator(self.client)
        ...
        *,
        ...
        try:
            with trace.stage("sql_generate"):
                sql = self.generator.generate(question, slots)
        ...
        with trace.stage("sql_render"):
            answer, number_ok = self.renderer.render(
                question, trace.generated_sql or "", result
            )
        ...
        try:
            with trace.stage("sql_repair"):
                repaired = self.generator.repair(
                    question, failed_sql, result.error or "", slots
                )
